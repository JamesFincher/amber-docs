...
import { loadAllDocs } from "../src/lib/content/docs.server";
import { loadGlossary } from "../src/lib/content/blocks.server";
...
async function validateExternalLinks(urls: string[], failures: Failure[]) {
  if (urls.length === 0) return;
  if (process.env.SKIP_EXTERNAL_LINKS === "1") return;

  const unique = Array.from(new Set(urls)).slice(0, 250);
  ...
  async function check(url: string) {
    ...
    try {
      ...
      fail("external_link", `External link failed (${head.status}/${get.status}): ${url}`, failures);
    } catch {
      fail("external_link", `External link failed (network): ${url}`, failures);
    } finally {
      clearTimeout(t);
    }
  }
...
async function main() {
  ...
  for (const d of docs) {
      ...
      if (link.version) {
        ...
        if (!exists) {
          ...
          fail(
            ...
            `Doc ${key} links to missing ${target}: ${link.raw}`,
            failures,
          );
        }
      } else {
        ...
        if (!exists) {
          fail(
            "broken_internal_link",
            `Doc ${key} links to missing /${link.kind}/${link.slug}: ${link.raw}`,
            failures,
          );
        }
      }
    ...
    for (const asset of findAssetLinks(d.markdown)) {
      const expected = path.join(process.cwd(), "public", asset.replace(/^\//, ""));
    ...
    if (d.stage === "official") {
      ...
      for (const g of glossary) {
        const term = g.term;
        const reAny = new RegExp(`\\b${term.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}\\b`, "i");
        if (!reAny.test(scan)) continue;
        const reExact = new RegExp(`\\b${term.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}\\b`, "g");
  ...
  for (const [slug, versions] of bySlug.entries()) {
    const sorted = [...versions].sort((a, b) => (a.updatedAt < b.updatedAt ? 1 : a.updatedAt > b.updatedAt ? -1 : 0));
  ...
  await validateExternalLinks(external, failures);
  ...
  console.log(`Content QA passed (${docs.length} doc versions).`);
}
