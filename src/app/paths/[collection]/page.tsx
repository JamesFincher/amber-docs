...
export function generateMetadata({ params }: { params: { collection: string } }): Metadata {
  return {
    title: `Path | ${params.collection} | Amber Protocol`,
  };
}
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 py-10">
      <header className="mb-8 space-y-3">
        ...
        <p className="max-w-3xl text-zinc-600">Recommended reading order. Each doc links to the latest version.</p>
      </header>

      <section className="card p-6">
        <ol className="grid gap-3">
          {c.docs.map((d, idx) => (
            <li key={d.slug} className="rounded-2xl border border-zinc-200 bg-white/70 p-4 backdrop-blur">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Step {idx + 1}</div>
